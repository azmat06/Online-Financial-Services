...
if(isset($_POST['bttn1'])){
	...
	if(empty($search)){
		header('location: feature5_viewtransbyname.php?error=emptyinput');
		exit();
	}
	$sql;
	if($_SESSION['acctype']=='user'){
		$sql="select a.fname, a.lname, m.merchant_id, t.transaction_id, t.date, t.service_id, s.s_name, s.price from (transactions t inner join merchants m on t.merchant_id=m.merchant_id inner join account a on m.merchant_id=a.account_id) inner join services s on t.service_id=s.service_id where t.user_id=".$_SESSION['accuid']."  and (a.fname REGEXP '".$search."' or  a.lname REGEXP '".$search."') order by t.date desc;";
	}else if($_SESSION['acctype']=='merchant'){
		$sql="select a.fname, a.lname, u.user_id, t.transaction_id, t.date, t.service_id, s.s_name, s.price from (transactions t inner join users u on t.user_id=u.user_id inner join account a on u.user_id=a.account_id) inner join services s on t.service_id=s.service_id where t.merchant_id=".$_SESSION['accuid']." and (a.fname REGEXP '".$search."' or  a.lname REGEXP '".$search."') order by t.date desc;";
	}
...
<body>
	...
	<section class="main">
		...
		<main id="mainshit" class="form-style">
			...
			<form action="feature5_viewtransbyname.php" method="post">
				<fieldset>
